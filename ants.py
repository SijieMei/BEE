...
import random
from ucsd import main, interact, trace
...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if type(self.ant) == QueenAnt and self.ant.tr == True:
                return
            if self.ant is insect:
                ...
                if hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
                    if type(self.ant.ant) == QueenAnt and self.ant.ant.tr == True:
                        return
                    self.ant.ant = None
                else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while temp_place.bees == []:
            temp_place = temp_place.entrance
        if temp_place == hive:
            return None
        return random_or_none(temp_place.bees)
        # END Problem 5
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self,hive):
        temp_place = self.place
        range = 0
        while (temp_place.bees == [] or range <= self.min_range) and temp_place != hive:
            temp_place = temp_place.entrance
            range += 1
        if temp_place == hive:
            return None
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self,hive):
        temp_place = self.place
        range = 0
        while (temp_place.bees == [] or range >= self.max_range) and temp_place != hive:
            temp_place = temp_place.entrance
            range += 1
        if temp_place == hive:
            return None
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.tr == True:
            ...
            while temp != None:
                if temp.ant != None:
                    if temp.ant.power == False:
                        temp.ant.power = True
                        temp.ant.damage = temp.ant.damage * 2
                    if temp.ant.container == True and temp.ant.ant != None:
                        if temp.ant.ant.power == False:
                            temp.ant.ant.power = True
                            temp.ant.ant.damage = temp.ant.ant.damage * 2
                temp = temp.exit
        else:   
